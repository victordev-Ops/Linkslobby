  ...
  if (feedback?.success) {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-gray-400 text-sm mb-8">Your secret is safe with us.</p>

        <div className="space-y-3">
          <button
            onClick={() => setFeedback(null)}
            className="w-full py-3 px-6 bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold rounded-2xl transition-all text-sm active:scale-95"
          >
            Send another
          </button>
          ...
          <Link
            href="/signup"
            className="flex items-center gap-3 p-4 rounded-2xl bg-purple-600 hover:bg-purple-700 transition-all active:scale-95 text-left"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-purple-200 text-xs mt-0.5">Receive anonymous confessions</p>
            </div>
    ...
    <form ref={formRef} action={handleSubmit} className="space-y-3">
      ...
      <div className="relative">
        <textarea
          ...
          className="w-full bg-gray-50 text-gray-900 placeholder-gray-400 rounded-2xl p-4 border border-gray-200 focus:border-purple-400 focus:ring-4 focus:ring-purple-500/10 outline-none transition-all resize-none text-sm leading-relaxed"
          required
          minLength={1}
          maxLength={1000}
      ...
      <button
        ...
        className={`relative w-full py-4 rounded-2xl font-bold text-sm shadow-lg transition-all duration-200 overflow-hidden flex items-center justify-center gap-2
          ${isBlocked
            ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
            : 'bg-purple-600 hover:bg-purple-700 text-white shadow-purple-200 active:scale-[0.98] disabled:opacity-50'
          }`}
      >
        {isPending ? (
