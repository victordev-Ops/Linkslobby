...
import { Plus, Users, Play, Loader2, ArrowRight, X, Sparkles, Trash2, ChevronLeft, LayoutGrid, Share2, Crown } from 'lucide-react';
    ...
    const renderLobbyCard = (lobby: Lobby) => {
        ...
        return (
            <div
                ...
                className={`group relative overflow-hidden flex flex-col justify-between p-4 rounded-3xl transition-all cursor-pointer border snap-center w-[220px] shrink-0 h-[160px] border-green-500/30 hover:border-green-500/60 bg-gradient-to-br from-slate-900/80 to-slate-900/40 ${joiningLobbyId === lobby.id ? 'ring-2 ring-red-500' : ''}`}
            >
                <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-slate-500 truncate mt-0.5">
                            {isHost ? (
                                <span className="text-amber-400 font-semibold flex items-center gap-1"><Crown size={10} /> You host</span>
                            ) : (
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="mt-8 text-left">
                        ...
                        <p className="text-slate-500 text-sm font-medium">
                            {isPro ? `You can run up to ${PRO_LIMIT} lobbies at once` : `Create a lobby and invite friends with the link`}
                        </p>
                    </div>
                    ...
                    <div className="mt-10">
                        {sortedLobbies.length > 0 ? (
                            ...
                            <div className="text-center py-20 bg-slate-900/40 backdrop-blur-xl rounded-[3rem] border border-slate-800/50 mt-4">
                                ...
                                <h3 className="text-2xl font-black text-white mb-2">No lobbies yet</h3>
                                <p className="text-slate-500 text-sm max-w-xs mx-auto mb-6">
                                    Create a lobby and share the link to start playing with friends.
                                </p>
                                <button
                                    onClick={openCreateModal}
                                    className="bg-gradient-to-r from-red-500 to-orange-500 text-white px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-red-500/20 hover:shadow-red-500/40 transition-all active:scale-95 inline-flex items-center gap-2"
                                >
                                    <Plus size={16} />
                                    New Lobby
                                </button>
