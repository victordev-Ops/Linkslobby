...
import { Plus, Users, Clock, Crown, Play, Loader2, ArrowRight, X, Sparkles, Lock, Ban, Check, ChevronLeft, LayoutGrid, Trash2, Filter, Share2 } from 'lucide-react';
                                ...
                                <>
                                    ...
                                    {hasMorePrivate && (
                                        <button
                                            ...
                                            className="mt-3 w-full py-2.5 rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 transition-all font-bold flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
                                            {isLoadingMorePrivate ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
                                            <span className="text-[10px] uppercase tracking-wider">{isLoadingMorePrivate ? 'Loading...' : 'Load More Lobbies'}</span>
                                        </button>
                                    )}
                                </>
                            ) : (
                                ...
                                <>
                                    ...
                                    {hasMorePublic && (
                                        <button
                                            ...
                                            className="mt-3 w-full py-2.5 rounded-2xl bg-slate-900/40 border border-dashed border-slate-800/60 text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 transition-all font-bold flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
                                            {isLoadingMorePublic ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
                                            <span className="text-[10px] uppercase tracking-wider">{isLoadingMorePublic ? 'Loading...' : 'Load More Lobbies'}</span>
                                        </button>
                                    )}
                                </>
                            ) : (
