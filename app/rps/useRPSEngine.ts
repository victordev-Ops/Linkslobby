                    ...
                    if (currentMatch && currentMatch.ai_player && currentMatch.disconnected_player) {
                        reconnectToMatch(currentMatch.id).then(res => {
                            if (res.success && res.action === 'reconnected') {
                                setAiActive(false)
                                // Explicitly confirm to THIS player that their
                                // opponent — not the AI — is back in control.
                                toast.success("Opponent reconnected! They're back in control. 🎉", { id: "disconnect-toast" })
