...
export function useRPSEngine(profile: { id: string; username: string; slug: string; is_pro: boolean }) {
            ...
            .on("presence", { event: "join" }, ({ newPresences }) => {
                if (newPresences.some((p: any) => p.user_id !== profile.id)) {
                    ...
                    if (currentMatch && currentMatch.ai_player && currentMatch.disconnected_player) {
                        reconnectToMatch(currentMatch.id)
                    } else {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [supabase, profile.id, clearDisconnectTimers, startDisconnectSequence])
    ...
    useEffect(() => {
        ...
        if (nowAiPlayer && !prev.aiPlayer) {
            toast("AI has temporarily taken over for your opponent", { icon: "🤖", id: "disconnect-toast" })
        }
        ...
        if (nowAiPlayer && !nowDisconnected && prev.disconnectedPlayer) {
            toast.success("Opponent is back! AI is finishing this round…", { id: "disconnect-toast" })
        }

        // AI fully cleared — opponent has full control back
        if (!nowAiPlayer && prev.aiPlayer) {
            toast.success("Opponent is back in control! 🎮", { id: "disconnect-toast" })
        }
    ...
    useEffect(() => {
        ...
        const fire = () => {
            ...
            if (!m) return
            // Whichever side's move is still null when the clock runs out
            // is the one who gets defaulted. Server-side, the RPC re-checks
            // this against its own authoritative deadline before acting, so
            // it's safe even though either client may be the one to call it.
            const stallingId = !m.move_a ? m.player_a : (!m.move_b ? m.player_b : null)
            if (!stallingId) return // both moved already, nothing to enforce

            triggerAITakeover(m.id, stallingId).then(res => {
                if (res.success && (res.action === 'ai_takeover' || res.action === 'already_ai')) {
                    if (stallingId === profile.id) {
                        toast.error("You missed the 60s move window — your stake is forfeited. AI takes over your side.", { duration: 5000 })
                    } else {
                        toast.success("Opponent missed the move window! AI takes over their side.", { duration: 5000 })
                    }
                }
            })
        }
    ...
    }, [mode, match?.status, match?.move_deadline_at, match?.move_a, match?.move_b, aiActive, profile.id])
