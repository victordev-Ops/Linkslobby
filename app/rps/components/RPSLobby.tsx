        ...
        <div className="min-h-[100dvh] bg-[#0a0a0f] text-white relative overflow-hidden">
            ...
            <header className="sticky top-0 z-30 bg-[#0a0a0f]/80 backdrop-blur-md border-b border-white/5 px-3 py-2.5 flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <h1 className="font-bold text-sm truncate font-[family-name:var(--font-display)]">RPS Arena</h1>
                </div>
            ...
            <main className="max-w-md mx-auto px-4 pt-7 pb-10 space-y-7 relative z-10">
                <div className="text-center space-y-3">
                    <RPSHandsHero />
                    <h2 className="text-2xl sm:text-3xl font-black bg-gradient-to-r from-emerald-400 to-teal-300 bg-clip-text text-transparent font-[family-name:var(--font-display)]">
                        Rock Paper Scissors
                    </h2>
                    <p className="text-white/50 text-sm max-w-[260px] mx-auto">Best of 5 — first to 3 takes the win (and the bragging rights).</p>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left min-w-0">
                                <h3 className="font-bold text-base sm:text-lg text-white font-[family-name:var(--font-display)]">Solo Showdown</h3>
                                <p className="text-xs text-white/40">Warm up against the bot</p>
                            </div>
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left min-w-0">
                                <h3 className="font-bold text-base sm:text-lg text-white font-[family-name:var(--font-display)]">Challenge a Friend</h3>
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left min-w-0">
                                <h3 className="font-bold text-base sm:text-lg text-white font-[family-name:var(--font-display)]">Join the Duel</h3>
                                <p className="text-xs text-white/40">Got an invite? Drop the code</p>
                            </div>
                        </div>
                    </button>
                    ...
                    {engine.showJoinInput && (
                        <div className="flex gap-2 animate-in slide-in-from-top-2 duration-200">
                            ...
                            />
                            <button
                                onClick={() => engine.handlePeekRoom(engine.joinRoomId)}
                                disabled={engine.joinRoomId.length !== 6 || engine.isLoading || engine.isPeekingRoom}
                            ...
                            >
                                {engine.isPeekingRoom ? <Loader2 size={18} className="animate-spin" /> : "Join"}
                            </button>
                        </div>
                    )}
                </div>
                    ...
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-6">
                        <div className="bg-[#14141f] border border-white/10 rounded-t-3xl sm:rounded-3xl p-5 sm:p-6 w-full max-w-md shadow-2xl flex flex-col max-h-[85vh]">
                            <div className="flex justify-between items-center mb-5 sm:mb-6">
                                <h3 className="font-bold text-lg sm:text-xl text-white font-[family-name:var(--font-display)]">Battle Log</h3>
                                <button
                            ...
                            <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                                {engine.isLoadingHistory ? (
                                    ...
                                    <div className="text-center py-12 space-y-2">
                                        ...
                                        <p className="text-white/40 text-sm">No battles yet — go make some history</p>
                                    </div>
                                ) : (
                ...
                {/* Room Preview Modal — shows the stake BEFORE any Stars are locked */}
                {engine.roomPreview && (
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-6">
                        <div className="bg-[#14141f] border border-white/10 rounded-t-3xl sm:rounded-3xl p-6 max-w-sm w-full space-y-5 shadow-2xl">
                            <div className="text-center space-y-2">
                                ...
                                <h3 className="text-xl font-black text-white font-[family-name:var(--font-display)]">{engine.roomPreview.host_name}{"'"}s Duel</h3>
                                <p className="text-white/50 text-sm">
                                    Best of {engine.roomPreview.best_of} — stakes{" "}
                                    <span className="text-amber-400 font-bold inline-flex items-center gap-1">
                                        {engine.roomPreview.stake_amount} <Star size={12} fill="currentColor" />
                                    </span>{" "}
                                    per player.
                                </p>
                                <p className="text-white/30 text-xs">
                                    Joining locks {engine.roomPreview.stake_amount} Stars as escrow.
                                </p>
                            </div>
                            <div className="space-y-3">
                                <button
                                    onClick={engine.confirmJoinRoom}
                                    disabled={engine.isLoading}
                                ...
                                >
                                    {engine.isLoading ? (
                                        ...
                                        <>Join & Lock {engine.roomPreview.stake_amount} <Star size={14} fill="currentColor" /></>
                                    )}
                                </button>
                                <button
                                    onClick={engine.cancelJoinPreview}
                                    disabled={engine.isLoading}
                    ...
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-6">
                        <div className="bg-[#14141f] border border-white/10 rounded-t-3xl sm:rounded-3xl p-6 max-w-sm w-full space-y-5 shadow-2xl">
                            <div className="text-center space-y-2">
                                ...
                                <h3 className="text-xl font-black text-white font-[family-name:var(--font-display)]">Not Enough Stars!</h3>
                                <p className="text-white/50 text-sm">
                                    You need <span className="text-amber-400 font-bold">100 Stars</span> to play.
                                    You{"'"}ve got <span className="text-amber-400 font-bold">{engine.starBalance} Stars</span>.
                                </p>
                            </div>
                            ...
                            {engine.starBalance !== null && engine.starBalance > 0 && engine.pendingMode === "solo" ? (
                                ...
                                <div className="space-y-3">
                                    <p className="text-center text-white/40 text-sm">Rack up Stars by playing other games!</p>
                                    <button onClick={() => engine.setShowBalanceGate(false)} className="w-full py-3 text-white/40 hover:text-white/60 font-bold text-sm transition">
                                        Close
                                    </button>
                                </div>
