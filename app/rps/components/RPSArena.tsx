...
export function RPSArena({ engine }: { engine: ReturnType<typeof useRPSEngine> }) {
    ...
    const baseOpponentName = engine.mode === "solo" ? "Computer" : (engine.opponentName || "Opponent")
    const opponentLabel = engine.mode === "friend" && engine.aiActive ? `${baseOpponentName} (AI)` : baseOpponentName
    ...
    useEffect(() => {
        // matchResult flips to 'won' as soon as the server resolves the
        // deciding round — which can be BEFORE the countdown/reveal
        // animation for that round has even played. Confetti must only
        // start once the player is actually looking at the win (the
        // matchEnd screen), a few ms after it appears — not before.
        if (engine.matchResult === 'won' && engine.phase === 'matchEnd') {
            const id = window.setTimeout(startConfetti, 150)
            return () => window.clearTimeout(id)
        }
        // if matchResult changed away from 'won', or we haven't reached
        // matchEnd yet, ensure confetti is stopped
        stopConfetti()
        return
    }, [engine.matchResult, engine.phase])
                    ...
                    {engine.mode === "friend" && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 rounded-full border border-emerald-500/20">
                            ...
                            <span className="text-[10px] font-bold text-emerald-400">
                                vs {baseOpponentName} {engine.aiActive && '🤖'}
                            </span>
                        </div>
                    )}
                    ...
                    <div className="flex flex-col items-center justify-center py-16 space-y-6 animate-in fade-in duration-300">
                        ...
                        {engine.aiActive && (
                            <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-2xl p-4 text-center space-y-2 w-full max-w-xs">
                                {engine.match?.disconnected_player ? (
                                    <>
                                    ...
                                    <>
                                        <p className="text-emerald-400 font-bold text-sm">🎉 Opponent is back!</p>
                                        <p className="text-white/50 text-xs">AI is finishing this round — they're back in control next round.</p>
                                    </>
                                )}
                            </div>
                        )}
                        ...
                        <div className="text-center space-y-2">
                            ...
                            <p className="text-white/40 text-sm font-bold uppercase tracking-widest">
                                {engine.aiActive
                                    ? (engine.match?.disconnected_player ? "AI is thinking…" : "AI finishing this round…")
                                    : engine.mode === "solo" ? "Resolving..." : `${opponentLabel} is thinking…`}
                            </p>
                        </div>
                    </div>
                )}
