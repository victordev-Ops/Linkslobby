      ...
      <main className="max-w-xl mx-auto px-4 py-8 space-y-8 relative z-10">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-2">
            <LayoutGrid size={18} className="text-purple-600 dark:text-purple-400 icon-rotate-pause" />
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            {profile.is_pro ? (
              <div className="relative rounded-2xl border border-blue-200 dark:border-blue-500/30 bg-gradient-to-br from-blue-50 via-white to-slate-50 dark:from-blue-950/40 dark:via-transparent dark:to-slate-900/50 p-4 space-y-3 overflow-hidden animate-in fade-in slide-in-from-bottom-3 duration-500 delay-500 shadow-sm">
                <div className="flex items-center gap-2 px-1">
                  <BadgeCheck size={16} className="text-blue-500 dark:text-blue-400" fill="currentColor" stroke="white" strokeWidth={2} />
                  <h3 className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-[0.2em] font-[family-name:var(--font-display)]">Pro Games Unlocked</h3>
                </div>

                {/* Three Word Game */}
                <div className="group bg-white dark:bg-[#1a1429]/50 rounded-xl border border-slate-100 dark:border-white/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 p-3.5 flex items-center gap-3">
                  <div className="w-11 h-11 shrink-0 rounded-xl bg-cyan-100 dark:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 flex items-center justify-center group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300">
                    <Type size={20} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm font-[family-name:var(--font-display)]">Three Word Game</h4>
                      <span className="bg-blue-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase">Pro</span>
                    </div>
                    <p className="text-slate-400 dark:text-white/60 text-[11px] line-clamp-1">Describe me in exactly 3 words</p>
                  </div>
                  <button
                    onClick={handleNavigateToThreeWord}
                    disabled={isNavigatingToThreeWord}
                    className="shrink-0 px-3 py-2 bg-cyan-600 text-white text-[10px] font-bold rounded-lg hover:bg-cyan-700 transition-all active:scale-95 hover:scale-[1.02] disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {isNavigatingToThreeWord ? <Loader2 size={13} className="animate-spin" /> : 'Play'}
                  </button>
                </div>

                {/* Word Scramble */}
                <div className="group bg-white dark:bg-[#1a1429]/50 rounded-xl border border-slate-100 dark:border-white/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 p-3.5 flex items-center gap-3">
                  <div className="w-11 h-11 shrink-0 rounded-xl bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 flex items-center justify-center group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300">
                    <Shuffle size={20} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm font-[family-name:var(--font-display)]">Word Scramble</h4>
                      <span className="bg-blue-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase">Pro</span>
                    </div>
                    <p className="text-slate-400 dark:text-white/60 text-[11px] line-clamp-1">Unscramble the word before time's up</p>
                  </div>
                  <button
                    onClick={handleNavigateToScramble}
                    disabled={isNavigatingToScramble}
                    className="shrink-0 px-3 py-2 bg-violet-600 text-white text-[10px] font-bold rounded-lg hover:bg-violet-700 transition-all active:scale-95 hover:scale-[1.02] disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {isNavigatingToScramble ? <Loader2 size={13} className="animate-spin" /> : 'Play'}
                  </button>
                </div>
              </div>
            ) : (
