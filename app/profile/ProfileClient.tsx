...
import { ArrowLeft, Save, Copy, Check, Loader2, User as UserIcon, XCircle, CheckCircle, Camera, UserPlus, MessageCircle, Users, X, Clock, BadgeCheck, Send, ChevronDown, Search } from 'lucide-react'
        ...
        <div className={`${display.variable} ${body.variable} min-h-screen bg-[#F8F9FD] dark:bg-[#0f0a1e] transition-colors duration-300 pb-24 font-[family-name:var(--font-body)]`}>
            ...
            <main className="max-w-2xl mx-auto px-0 sm:px-4 -mt-16 relative z-10 space-y-6">
                ...
                <div className="bg-white dark:bg-[#1a1429]/80 dark:backdrop-blur-xl sm:rounded-3xl shadow-sm border-b sm:border border-slate-200 dark:border-white/10 pt-4 pb-6 px-6">
                    ...
                    {/* Stats Row */}
                    {!isEditing && (
                        <div className="flex justify-start gap-8 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/60">
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800/60 -mx-6 px-6 mt-2">
                        ...
                        <div
                            onClick={() => setIsFriendsSectionOpen(!isFriendsSectionOpen)}
                            className="flex items-center gap-3 py-4 cursor-pointer select-none"
                        >
                            <div className="w-9 h-9 shrink-0 rounded-xl bg-purple-100 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 flex items-center justify-center">
                                <Users size={18} />
                            </div>
                            <h3 className="font-bold text-slate-900 dark:text-white text-base flex-1 min-w-0 font-[family-name:var(--font-display)]">
                                Friends
                                {friends.length > 0 && (
                                    <span className="ml-1.5 text-slate-400 dark:text-white/40 font-semibold text-sm">{friends.length}</span>
                                )}
                            </h3>
                            <ChevronDown size={18} className={`text-slate-300 dark:text-white/30 transition-transform duration-300 ${isFriendsSectionOpen ? "rotate-180" : ""}`} />
                        </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <div className="relative flex-1">
                                        ...
                                        >
                                                ...
                                                {(() => {
                                                    ...
                                                    return (
                                                        <>
                                                            ...
                                                            {current.label}
                                                            {current.count > 0 && (
                                                                <span className="px-1.5 py-0.5 rounded-md text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400">
                                                                    {current.count}
                                                                </span>
                                                            )}
                                                        </>
                                            ...
                                            <ChevronDown size={16} className={`transition-transform duration-200 ${isFriendsDropdownOpen ? "rotate-180" : ""}`} />
                                        </button>
                                        ...
                                        {filteredFriends.map(f => (
                                            <div key={f.id} className="flex items-center gap-3 p-3 sm:p-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors rounded-xl">
                                                ...
                                                <button
                                                    ...
                                                    className="w-10 h-10 rounded-full flex items-center justify-center bg-slate-100 hover:bg-purple-100 text-slate-600 hover:text-purple-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-purple-500/20 dark:hover:text-purple-400 transition-colors flex-shrink-0"
                                                    title="Message"
                                                >
