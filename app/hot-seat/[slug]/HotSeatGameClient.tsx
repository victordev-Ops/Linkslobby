...
import {
    ...
    Timer, Zap, Plus, Minus, Rocket, ChevronUp
} from "lucide-react"
...
// "Today, 4:30 PM" / "Tomorrow, 9:00 AM" / "Fri, Jul 11, 9:00 AM" — used for
// timers scheduled far enough out that a bare countdown isn't useful.
...
// "in 45s" / "in 12m" / "in 2h 14m" / "in 1d 3h" — a human-friendly relative
// label that updates as the countdown ticks down, regardless of how far out
...
// Tomorrow at a given local hour (defaults to 9 AM) — used by the
// "Tomorrow" scheduling preset.
function tomorrowAt(hour: number = 9, minute: number = 0): Date {
    const d = new Date()
...
// Big synced countdown ring shown to host + participants while a start
// timer is running. Progress is driven by remaining/duration so it stays
// accurate even for someone who joined mid-countdown.
function CountdownRing({ remaining, duration }: { remaining: number; duration: number }) {
    const size = 128
    const stroke = 6
    ...
    return (
        <div className="relative w-32 h-32">
            <div className={`absolute inset-0 blur-2xl rounded-full ${urgent ? 'bg-red-500/30' : 'bg-amber-500/20'} animate-pulse`} />
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                <span className={`font-black text-4xl tabular-nums ${urgent ? 'text-red-400' : 'text-white'}`}>
                    {remaining}
                </span>
            </div>
        </div>
...
// One row in the mobile tools speed-dial: icon pill + label, sized for
// thumb taps (44px target) with an optional count badge.
function FabAction({ label, icon: Icon, onClick, badge }: { label: string; icon: ComponentType<{ size?: number; className?: string }>; onClick: () => void; badge?: number }) {
    return (
        <button
            onClick={onClick}
            className="flex items-center gap-2.5 pl-3.5 pr-2 py-2 rounded-full bg-[#1a1a2e] border border-white/10 shadow-xl active:scale-95 transition"
        >
            <span className="text-xs font-bold text-white/80 whitespace-nowrap">{label}</span>
            <span className="relative w-9 h-9 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0">
...
export default function HotSeatGameClient({ session, userProfile }: HotSeatGameClientProps) {
    ...
    // Start Timer / Countdown State — host picks how long everyone has to
    // get ready, then a synced countdown (driven off a shared timestamp,
    // not a locally-ticking number) plays for every participant so no one's
    // countdown drifts relative to anyone else's.
    const [startsAt, setStartsAt] = useState<string | null>(session.starts_at || null)
    const [countdownDuration, setCountdownDuration] = useState<number>(session.countdown_seconds || 30)
    ...
    const [isCancelingCountdown, setIsCancelingCountdown] = useState(false)
    const countdownTriggeredRef = useRef(false)
    // "Quick" = seconds/minutes from now, "Later" = hours out or a specific
    // scheduled clock time (later today, tomorrow, or a custom date/time)
    const [pickerTab, setPickerTab] = useState<'quick' | 'later'>('quick')
    const [selectedLaterHours, setSelectedLaterHours] = useState<number | null>(1)
    const [showCustomPicker, setShowCustomPicker] = useState(false)
    const [customDateTime, setCustomDateTime] = useState('')

    // Mobile "tools" FAB — surfaces the actions people actually reach for
    // mid-game (participants, invite, share) in the bottom thumb zone
    // instead of only in the top-corner header menu.
    ...
    // Close the tools FAB on outside click
    useEffect(() => {
        if (!fabOpen) return
        const handler = (e: MouseEvent) => {
    ...
    useEffect(() => {
        const channel = supabase.channel(`hot-seat:${session.id}`)
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'hot_seat_sessions', filter: `id=eq.${session.id}` }, (payload) => {
                ...
                setStartsAt(payload.new.starts_at ?? null)
                if (payload.new.countdown_seconds) setCountdownDuration(payload.new.countdown_seconds)
            })
    ...
    // Start-Timer Countdown Logic — ticks off the shared `startsAt`
    // timestamp (not a local counter) so a participant who joins mid-countdown
    // still sees the correct remaining time. For timers scheduled far out
    // (an hour, tomorrow, etc.) nobody may have the app open at the exact
    // moment it's due, so ANY client — host or participant — that's open
    // when the time passes (or opens the app afterward) calls a
    // SECURITY DEFINER RPC that idempotently flips the session to 'active'
    // only if it's actually due. Safe to call redundantly from many clients
    // at once since it's guarded by `starts_at <= now() and status = 'waiting'`.
    useEffect(() => {
        if (!startsAt || status !== 'waiting') {
        ...
        const tick = () => {
            const remainingMs = new Date(startsAt).getTime() - Date.now()
            const remaining = Math.max(0, Math.ceil(remainingMs / 1000))
            setCountdownRemaining(remaining)

            if (remaining <= 0 && !countdownTriggeredRef.current) {
                countdownTriggeredRef.current = true
                supabase.rpc('start_hot_seat_if_due', { p_session_id: session.id })
    ...
    }, [startsAt, status, session.id, supabase])

    // Schedules the session to start at an absolute point in time — used by
    // both the quick chips (seconds/minutes from now) and the "later" chips
    // (hours from now, tomorrow morning, or a custom date/time).
    const scheduleStart = async (target: Date) => {
        ...
        try {
            const startsAtIso = target.toISOString()
            const { error } = await supabase
                .from('hot_seat_sessions')
                .update({ starts_at: startsAtIso, countdown_seconds: seconds })
                .eq('id', session.id)
            if (error) throw error
            setStartsAt(startsAtIso)
            setCountdownDuration(seconds)
            countdownTriggeredRef.current = false
            setShowCustomPicker(false)
            toast.success(`Session set to start ${formatScheduledTime(startsAtIso)}`)
        } catch (e) {
            console.error('Failed to schedule start:', e)
            toast.error('Failed to set the timer')
        } finally {
    ...
    const cancelCountdown = async () => {
        setIsCancelingCountdown(true)
        try {
            const { error } = await supabase
                .from('hot_seat_sessions')
                .update({ starts_at: null, countdown_seconds: null })
                .eq('id', session.id)
            if (error) throw error
            setStartsAt(null)
        } catch (e) {
            console.error('Failed to cancel countdown:', e)
            toast.error('Failed to cancel the timer')
        } finally {
            setIsCancelingCountdown(false)
        }
    }
    ...
    showXPNotification(10, "Time Ran Out!", "spend", "Stars Lost")
    }
    ...
    try {
        ...
        // NEW: Trigger the toast directly
        showXPNotification(10, "Question Skipped", "spend", "Stars Spent")
    } finally {
        setIsSkipping(false)
    }
    }
    ...
    return (
        <div className="flex flex-col h-[100dvh] bg-[#0a0a0f] text-white overflow-hidden relative">
                ...
                <div className="max-w-md mx-auto space-y-8">
                    ...
                    {status === 'waiting' && (
                        <div className="flex flex-col items-center justify-center py-8 text-center space-y-6">
                            {startsAt ? (
                                // ── Timer running: same view for host and participants.
                                // Under a minute out, show the precision ring; further
                                // out, show the scheduled clock time + relative label
                                // so "tomorrow at 9 AM" reads naturally instead of as
                                // a giant number of seconds. ──
                                <>
                                    {countdownRemaining <= 60 ? (
                                        <>
                                            <CountdownRing remaining={countdownRemaining} duration={Math.min(countdownDuration, 60)} />
                                            <div className="space-y-1.5">
                                                <h3 className="font-black text-2xl text-white tabular-nums">
                                                    Starting in {countdownRemaining}s
                                                </h3>
                                                <p className="text-sm text-white/40 max-w-[280px] mx-auto">
                                                    Get your rapid-fire questions ready — everyone jumps in the moment the timer hits zero.
                                                </p>
                                            </div>
                                        </>
                                    ) : (
                                        <>
                                            <div className="relative">
                                                ...
                                                <div className="relative w-24 h-24 rounded-full bg-white/5 border border-amber-500/20 flex items-center justify-center">
                                            ...
                                            <div className="space-y-1.5">
                                                ...
                                                <p className="text-sm text-amber-400/80 font-bold">
                                                    Starts in {formatRelative(countdownRemaining)}
                                                </p>
                                                <p className="text-xs text-white/30 max-w-[260px] mx-auto">
                                                    Everyone in the session sees this same countdown — no need to keep the app open until it starts.
                                                </p>
                                            </div>
                                        </>
                                    )}
                                    {isHost && (
                                        <div className="flex items-center gap-2">
                                            <button
                                                onClick={startGame}
                                                className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl text-sm transition active:scale-95 flex items-center gap-1.5"
                                            >
                                                <Rocket size={15} /> Start Now
                                            </button>
                                            <button
                                                onClick={cancelCountdown}
                                                disabled={isCancelingCountdown}
                                                className="px-5 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 font-bold rounded-xl text-sm transition active:scale-95 disabled:opacity-50 flex items-center gap-1.5"
                                            >
                                                {isCancelingCountdown ? <Loader2 size={14} className="animate-spin" /> : <X size={15} />}
                                                Cancel
                                            </button>
                                        </div>
                                    )}
                                ...
                                // ── Host: pick a start timer, or jump straight in ──
                                <>
                                    <div className="relative">
                                        <div className="absolute inset-0 bg-amber-500/20 blur-2xl rounded-full animate-pulse" />
                                    ...
                                    <div className="space-y-2">
                                        <h3 className="font-bold text-xl text-white">Set a Start Timer</h3>
                                        <p className="text-sm text-white/40 max-w-[280px] mx-auto">
                                            Give everyone a heads-up window — from a few seconds to tomorrow morning — or jump straight in.
                                        </p>
                                    </div>
                                    ...
                                    <div className="w-full max-w-xs space-y-3">
                                        ...
                                        <div className="flex items-center gap-1 p-1 bg-white/5 rounded-xl">
                                            <button
                                                ...
                                                className={`flex-1 text-xs font-bold py-2 rounded-lg transition ${pickerTab === 'quick' ? 'bg-amber-500 text-white' : 'text-white/40 hover:text-white/70'}`}
                                            >
                                                Quick
                                            </button>
                                            <button
                                                onClick={() => setPickerTab('later')}
                                                className={`flex-1 text-xs font-bold py-2 rounded-lg transition ${pickerTab === 'later' ? 'bg-amber-500 text-white' : 'text-white/40 hover:text-white/70'}`}
                                            >
                                                Schedule Later
                                            </button>
                                        </div>
                                            ...
                                            <>
                                                ...
                                                <div className="flex items-center justify-center gap-3 py-1">
                                                    <button
                                                        ...
                                                        className="w-9 h-9 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 flex items-center justify-center transition active:scale-90"
                                                    >
                                                        <Minus size={15} />
                                                    </button>
                                                    <span className="font-mono font-black text-lg text-white w-16 text-center tabular-nums">
                                                        {selectedDuration}s
                                                    </span>
                                                    <button
                                                        onClick={() => setSelectedDuration(d => Math.min(300, d + 5))}
                                                        className="w-9 h-9 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 flex items-center justify-center transition active:scale-90"
                                                    >
                                                        <Plus size={15} />
                                                    </button>
                                                ...
                                                <button
                                                    onClick={() => scheduleStart(new Date(Date.now() + selectedDuration * 1000))}
                                                    disabled={isStartingCountdown}
                                                    className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 rounded-2xl transition shadow-xl shadow-amber-500/20 flex items-center justify-center gap-2 disabled:opacity-50"
                                                >
                                                    {isStartingCountdown ? <Loader2 size={18} className="animate-spin" /> : <Timer size={18} />}
                                                    Start Timer
                                                </button>
                                            </>
                                        ) : (
                                            <>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {[
                                                        ...
                                                        <button
                                                            key={opt.label}
                                                            onClick={() => { setSelectedLaterHours(opt.hours); setShowCustomPicker(false) }}
                                                            className={`py-2.5 px-2 rounded-xl text-xs font-bold transition active:scale-95 ${
                                                                !showCustomPicker && selectedLaterHours === opt.hours
                                                                    ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20'
                                                                    : 'bg-white/5 text-white/50 border border-white/10 hover:bg-white/10'
                                                            }`}
                                                        >
                                                ...
                                                >
                                                    <Timer size={13} /> Custom date & time
                                                </button>
                                                ...
                                                {showCustomPicker && (
                                                    <input
                                                        ...
                                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white text-[15px] focus:outline-none focus:border-amber-500/50 transition"
                                                    />
                                                )}
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (showCustomPicker) {
                                                            if (!customDateTime) { toast.error('Pick a date and time first'); return }
                                                            scheduleStart(new Date(customDateTime))
                                                        } else if (selectedLaterHours === null) {
                                                            scheduleStart(tomorrowAt(9, 0))
                                                        } else {
                                                            scheduleStart(new Date(Date.now() + selectedLaterHours * 3600 * 1000))
                                                        }
                                                    }}
                                                    disabled={isStartingCountdown}
                                                    className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 rounded-2xl transition shadow-xl shadow-amber-500/20 flex items-center justify-center gap-2 disabled:opacity-50"
                                        ...
                                        <button
                                            onClick={startGame}
                                            className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 font-bold py-3 rounded-2xl transition flex items-center justify-center gap-2 text-sm"
                                        >
                                            <Play size={15} fill="currentColor" /> Skip Timer, Start Now
                                        </button>
                                    </div>
                                </>
                            ) : (
                                // ── Participant: no timer set yet ──
                                <>
                                    <div className="relative">
                                        <div className="absolute inset-0 bg-amber-500/20 blur-2xl rounded-full animate-pulse" />
                                        <div className="relative w-24 h-24 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-sm text-white/40 max-w-[280px] mx-auto">
                                            The session begins once the host starts a timer or jumps straight in. Get ready!
                                        </p>
                                    </div>
                                </>
                            )}
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5 max-w-sm w-full mx-auto">
                                <p className="text-xs text-white/60">
                                    💡 Tip: Think of tough questions while you wait! You can earn Stars if the host fails to answer.
                                </p>
                            </div>
                        </div>
                    )}
            ...
            {/* Mobile Tools FAB — thumb-reachable quick access to participants,
                invite and share, rather than only tucked in the top-corner menu. */}
            {status !== 'finished' && (
                <div ref={fabRef} className={`fixed right-4 z-30 flex flex-col items-end gap-2.5 ${!isHost && status === 'active' ? 'bottom-24' : 'bottom-6'} safe-area-bottom`}>
                    <AnimatePresence>
                        {fabOpen && (
                            <motion.div
                            ...
                            >
                                {isHost && status === 'active' && !currentQuestion && pendingQuestions.length > 0 && (
                                    <FabAction label="Next Question" icon={SkipForward} onClick={() => { activateNextQuestion(); setFabOpen(false) }} />
                                )}
                                {isHost && (
                                    <FabAction label="Invite Friends" icon={UserPlus} onClick={() => { setShowInviteFriends(true); setFabOpen(false) }} />
                                )}
                                <FabAction label="Share Link" icon={Share2} onClick={() => { shareLink(); setFabOpen(false) }} />
                                <FabAction
                                    label="Participants"
                                    icon={Users}
                                    badge={participantCount}
                                    onClick={() => { setShowParticipants(true); setParticipantsTab('active'); setFabOpen(false) }}
                                />
                            </motion.div>
                        )}
                    </AnimatePresence>
                    <button
                        ...
                        className="w-14 h-14 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 text-white shadow-xl shadow-amber-900/30 flex items-center justify-center active:scale-90 transition"
                        title="Tools"
                    >
                        <motion.div animate={{ rotate: fabOpen ? 45 : 0 }} transition={{ duration: 0.15 }}>
                            {fabOpen ? <X size={22} /> : <Zap size={22} fill="currentColor" />}
                        </motion.div>
                    </button>
                </div>
            )}
