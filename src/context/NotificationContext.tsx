  ...
  const refreshUnreadCount = useCallback(async () => {
    ...
    if (chatUnreadRes.data && chatUnreadRes.data.length > 0) {
      ...
      if (activeSessionsError) {
        console.error('[refreshUnreadCount] query failed: chat_sessions', activeSessionsError)
        logDebug('query failed: chat_sessions', activeSessionsError?.message || activeSessionsError)
      }
      ...
      logDebug('unreadMessagesCount computed', {
        sessionsWithParticipation: sessionIds.length,
        activeSessionsReturned: activeSessions?.length ?? null,
        totalUnread
      })
      setUnreadMessagesCount(totalUnread)
    } else {
      logDebug('unreadMessagesCount set to 0', { chatUnreadRowCount: chatUnreadRes.data?.length ?? null })
      setUnreadMessagesCount(0)
    }
  }, [profileId, supabase])
