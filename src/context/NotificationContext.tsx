...
export function NotificationProvider({
  ...
  const refreshUnreadCount = useCallback(async () => {
    ...
    const [
      ...
      turnRes, gameInviteRes, friendReqRes, friendRespRes, lobbyRespRes, hotSeatAnsRes, readsRes
    ] = await Promise.all([
    ...
    // deleteNotification (see notificationTypeKey in actions/notifications.ts) — so a
    // deleted notification is excluded from the count no matter which table it came from.
    ...
    const confessionsCount = (confRes.data || []).filter(r => !hiddenIds.has(`confession:${r.id}`)).length
    const dykmCount = (dykmRes.data || []).filter(r => !hiddenIds.has(`dykm_score:${r.id}`)).length
    const xpCount = (xpRes.data || []).filter(r => !hiddenIds.has(`xp_transaction:${r.id}`)).length
    const hotSeatCount = (hotSeatRes.data || []).filter(r => !hiddenIds.has(`hot_seat_question:${r.id}`)).length
    const turnCount = (turnRes.data || []).filter(r => !hiddenIds.has(`tod_turn_event:${r.id}`)).length
    const gameInviteCount = (gameInviteRes.data || []).filter(r => !hiddenIds.has(`game_invite:${r.id}`)).length
