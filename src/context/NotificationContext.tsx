...
const NOTIFICATION_TYPE_KEY: Record<string, string> = {
  ...
  // FIX: these two were never added here even after hidden_notifications'
  // check constraint (and DELETABLE_TYPES in actions/notifications.ts) started
  // allowing them — so a deleted lobby_join_response/hot_seat_answer kept
  // counting toward the bell badge forever, since the hiddenIds.has(...) checks
  // below never had a matching key to look up. See lobbyRespCount/hotSeatAnsCount.
...
export function NotificationProvider({
  ...
  const refreshUnreadCount = useCallback(async () => {
    ...
    // FIX: both of these only checked readIds before — a deleted (hidden)
    // lobby_join_response or hot_seat_answer kept counting toward the badge
    // forever, since nothing ever subtracted hiddenIds for them. Now that
    // deleteNotification() actually supports hiding these two types (see
    // actions/notifications.ts), the count has to respect it too.
    const lobbyRespCount = (lobbyRespRes.data || []).filter(r =>
      !readIds.has(`lobby_join_response:${r.id}`) && !hiddenIds.has(`${NOTIFICATION_TYPE_KEY.lobby_join_response}:${r.id}`)
    ).length
    const hotSeatAnsCount = (hotSeatAnsRes.data || []).filter(r =>
      !readIds.has(`hot_seat_answer:${r.id}`) && !hiddenIds.has(`${NOTIFICATION_TYPE_KEY.hot_seat_answer}:${r.id}`)
    ).length
