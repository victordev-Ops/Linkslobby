  ...
  toast.custom(
    (t) => (
      <div
        ...
        className="relative flex items-start gap-2.5 py-2.5 pl-3 pr-2 rounded-xl border shadow-md bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-50 border-zinc-200/80 dark:border-white/10 w-[min(320px,calc(100vw-24px))] cursor-pointer"
      >
        {Icon && (
        ...
        <button
          ...
          className="shrink-0 mt-0.5 min-w-9 min-h-9 flex items-center justify-center rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
    ...
    { id: toastId, duration, dismissible: true }
  )
...
export function showAppError(title: string, description?: string) {
  return showAppToast(title, { variant: 'error', description, duration: 4000 })
}
