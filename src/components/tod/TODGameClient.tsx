...
import { Loader2, AlertCircle, Users, Share2, Sparkles, Play, StopCircle, X, ArrowLeft, Timer, Clock, Trash2, LayoutGrid, ChevronLeft, LogOut, MoreVertical, Dices } from "lucide-react";
...
export default function TODGameClient({ lobbyId }: TODGameClientProps) {
  ...
  const handleInviteFriend = useCallback(async (friendUserId: string) => {
    ...
    if (result.success) {
      ...
      showAppSuccess('Invite sent! 🎉');
    } else {
      showAppError(result.error || 'Failed to send invite');
    }
  }, [lobby]);
  ...
  useEffect(() => {
    if (isBanned && !isLoading) {
      showAppError('You have been banned from this lobby.');
      router.refresh();
      router.push('/tod');
    }
  ...
  const copyInviteLink = async () => {
    ...
    try {
      if (navigator.share && navigator.canShare?.(shareData)) {
        ...
        showAppSuccess("Link copied! Share with friends 🎉");
      }
    } catch (err: any) {
  ...
  const handleSendMessage = async (content: string, imageUrl: string | null) => {
    ...
    await sendMessage(finalContent, imageUrl, messageType, profile?.username, questionRef, profile?.avatar_url, profile?.is_pro);
    setReplyingTo(null);
  ...
  const handleLeaveLobby = async () => {
    ...
    if (!isHost && !isJoined) {
      ...
      setIsLeaving(false);
      if (!ok) return; // stay put — the lobby wasn't actually left
      cleanup();
      router.refresh();
      router.push('/tod');
      return;
  ...
  const confirmLeave = async () => {
    setIsLeaving(true);
    try {
      const ok = isHost ? await deleteLobby() : await leaveLobby();
      if (!ok) {
        // RPC failed — a toast was already shown by useGameLogic.
        // Don't navigate away or the UI will look "successful" while
        // the lobby/participant row is untouched in the database.
        setIsLeaving(false);
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-slate-950 overflow-hidden font-sans selection:bg-red-500/30 overscroll-behavior-none touch-none">
      ...
      <div className="relative z-10 w-full max-w-lg h-full flex flex-col bg-slate-950 border-x border-white/5 shadow-2xl overflow-hidden">
...

        {
          !isJoined && !isBanned && !isLoading && (
            <div className="absolute inset-0 z-[110] flex flex-col items-center justify-center p-6 text-center backdrop-blur-md bg-slate-950/40">
              ...
              <h2 className="text-3xl font-black text-white mb-4 italic">Reconnecting...</h2>
              <p className="text-slate-400 max-w-md leading-relaxed mb-8">
                We couldn't confirm you as a participant in this lobby. This is usually
                a brief sync delay — hang tight, or go back and rejoin from the link.
              </p>
              <button
                onClick={handleLeaveLobby}
                className="px-8 py-3 rounded-full bg-slate-800 border border-slate-700 text-white font-bold hover:bg-slate-700 transition active:scale-95"
        ...
        {/* Top Header - STICKY, with safe-area padding for notched phones */}
        <header
          className="sticky top-0 z-[100] flex-shrink-0 px-3 sm:px-4 py-3 backdrop-blur-xl bg-slate-900/80 border-b border-white/5 shadow-lg"
          style={{ paddingTop: 'max(0.75rem, env(safe-area-inset-top))' }}
        >
          <div className="flex items-center justify-between gap-3 relative">
            {/* Left: My avatar + Participants roster access */}
            <div className="flex items-center justify-start flex-1">
              <button
                onClick={() => setShowSidebar(!showSidebar)}
                className="relative w-11 h-11 rounded-full flex items-center justify-center shadow-lg active:scale-95 transition-transform min-w-[44px] min-h-[44px]"
                aria-label="Open players list"
              >
                <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-red-500/60 bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center">
                  {profile?.avatar_url ? (
            ...
            <div className="flex items-center justify-end flex-1">
              <button
                ...
                className="w-11 h-11 min-w-[44px] min-h-[44px] rounded-full bg-slate-800 border border-slate-700 hover:bg-slate-700 text-white flex items-center justify-center transition active:scale-95"
