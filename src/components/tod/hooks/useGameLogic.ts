...
export const useGameLogic = (lobbyId: string, userId?: string) => {
  ...
  const collectOnlineIds = (state: ReturnType<RealtimeChannel['presenceState']>) => {
    const onlineIds = new Set<string>();
    for (const [presenceKey, presences] of Object.entries(state)) {
      if (presenceKey && presenceKey !== 'undefined') onlineIds.add(presenceKey);
      (presences as any[]).forEach((p) => {
        const id = p.user_id || p.key;
        if (id && id !== 'undefined') onlineIds.add(id);
      });
    }
    return onlineIds;
  };

  const cancelLeaveTimer = (id: string) => {
  ...
  useEffect(() => {
    ...
    // Do NOT key this channel on userId. TODGameClient passes profile?.id,
    // which is undefined on first paint and arrives a moment later — putting
    // userId in this effect's deps tore the presence channel down (a leave
    // event for everyone already in the lobby) and recreated it, which is
    // the "everyone flashes offline" race. Subscribe once per lobby; track
    // presence separately when userId becomes available.
    const gameChannel = supabase.channel(`game:${lobbyId}`);
    channelRef.current = gameChannel;

    const PRESENCE_LEAVE_GRACE_MS = 4000;
    ...
    gameChannel
      ...
      .on('presence', { event: 'sync' }, () => {
        const onlineIds = collectOnlineIds(gameChannel.presenceState());
        setOnlineUsers(prev => {
          // Empty sync during a reconnect/remount is transient — keep the
          // previous set until a real leave (after grace) confirms it.
          if (onlineIds.size === 0 && prev.size > 0) return prev;
          return onlineIds;
        });
      })
      .on('presence', { event: 'join' }, ({ key, newPresences }) => {
        ...
        (newPresences as any[] | undefined)?.forEach((p) => {
          const id = p.user_id || p.key;
          if (id && id !== 'undefined') ids.add(id);
        });
        ids.forEach(cancelLeaveTimer);
      ...
      .on('presence', { event: 'leave' }, ({ key, leftPresences }) => {
        const ids = new Set<string>();
        if (key && key !== 'undefined') ids.add(key);
        (leftPresences as any[] | undefined)?.forEach((p) => {
          const id = p.user_id || p.key;
          if (id && id !== 'undefined') ids.add(id);
        });
        ids.forEach((id) => {
          ...
          const timer = setTimeout(() => {
            leaveTimersRef.current.delete(id);
            const stillOnline = collectOnlineIds(gameChannel.presenceState());
            if (!stillOnline.has(id)) {
              setOnlineUsers(prev => {
      ...
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          if (id) {
            await gameChannel.track({ user_id: id, key: id, online_at: new Date().toISOString() });
          }
        }
      });
    ...
    return () => {
      ...
      if (channelRef.current === gameChannel) channelRef.current = null;
      setChannel(null);
    };
  }, [lobbyId, supabase, fetchData]);

  // Track presence once profile.id arrives, without tearing the channel down.
  useEffect(() => {
    if (!userId || !channel) return;
    channel.track({ user_id: userId, key: userId, online_at: new Date().toISOString() }).catch(() => {});
  }, [userId, channel]);
