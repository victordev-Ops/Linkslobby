...
interface DirectMessageClientProps {
    ...
    // Prefetched server-side by the page component (see messages/[id]/page.tsx)
    // so the very first render already has real messages instead of a
    // spinner — this is what removes the client-only fetch waterfall that
    // used to run on every fresh thread open.
    initialMessages?: any[]
...
// Relative "last seen" label for the offline state in the header
...
function mapServerMessage(m: any, currentUserId: string): Message {
...
export default function DirectMessageClient({
    ...
    // Seed directly from the server-prefetched page prop when present — real,
    // fresh data from moments ago in the same request, strictly better than
    // either the Dexie cache or a blank array as a starting point.
    const [messages, setMessages] = useState<Message[]>(() =>
        (initialMessages || []).map(m => mapServerMessage(m, currentUser.id))
    )
    const [inputText, setInputText] = useState('')
    const [isUploading, setIsUploading] = useState(false)
    // No spinner on first paint when server-prefetched messages are already
    // here — only fall back to loading when we truly have nothing to show.
    ...
    // Calibrates optimistic message timestamps against the SERVER's clock,
    // not the device's. groupMessages() breaks a group on a >2min gap by
    // comparing created_at values — for confirmed messages both sides are
    // server time, so that's fine, but an optimistic message's created_at
    // was previously the device's own `Date.now()`. Any device/server clock
    // skew (common on phones) could make that gap check wrongly fire before
    // the message is confirmed and its timestamp gets corrected — which is
    // exactly what made grouping look inconsistent rather than reliably
    // wrong: it self-corrected a moment later, so it looked fine on some
    // devices/sends and broken on others. This ref is updated every time a
    // confirmed (server) message is seen, and used to offset Date.now() for
    // any optimistic message going forward.
    const clockOffsetRef = useRef(0)

    // Typing State
    const [replyingTo, setReplyingTo] = useState<Message | null>(null)
    // Swiping to reply on a message that's still "optimistic" (sent a
    // moment ago, not yet confirmed by the server) is a completely normal
    // thing to do — replying to something you just sent is a common
    // pattern. The message only has a temp-* client id at that point
    // though, so we can't send that as reply_to_id. Instead of blocking the
    // gesture (which just made the reply feature feel broken/unresponsive),
    // we let replyingTo point at the optimistic message, then patch its id
    // in place here the moment the real id lands — from whichever of the
    // three places that happens (this tab's own send confirming, or the
    // realtime broadcast confirming a message sent from this tab). A ref is
    // needed alongside the state because two of those three places are
    // long-lived callbacks (the realtime subscription) that would otherwise
    // only ever see the replyingTo value from whenever that effect was set
    // up, not whatever the user is currently replying to.
    const replyingToRef = useRef<Message | null>(null)
    useEffect(() => { replyingToRef.current = replyingTo }, [replyingTo])
    const reconcileReplyingToId = (tempId: string, realId: string) => {
        if (replyingToRef.current?.id === tempId) {
            setReplyingTo(prev => (prev && prev.id === tempId) ? { ...prev, id: realId, isOptimistic: false } : prev)
        }
    }
    ...
    // Keep layout height AND position in sync with the visual viewport so the
    // on-screen keyboard never covers the input area. Height alone isn't
    // enough: on iOS, scrolling while the keyboard is open shifts the visual
    // viewport's offsetTop without necessarily changing its height, and a
    // container sized only by height (still positioned at the top of the
    // document flow) ends up misaligned with what's actually visible —
    // that's what was showing up as a blank gap between the input bar and
    // the keyboard. Tracking both, on both 'resize' and 'scroll', keeps the
    // container pinned to the true visible area.
    useEffect(() => {
    ...
    // Force a re-render every 30s so the "last seen Xm ago" label in the
    // header stays accurate without needing a fresh presence event.
    ...
    useEffect(() => {
        ...
        const hasServerPrefetch = !!initialMessages

        // Scroll straight to the bottom immediately — no need to wait for
        // the revalidation fetch below, we already have real messages to show.
        ...
        const loadInitial = async () => {
            ...
            if (!hasServerPrefetch) try {
                ...
                if (recentCached.length > 0) {
                    ...
                    setIsSyncing(true) // ...but keep a lightweight sync indicator until the server responds
            ...
            if (result.success && result.data) {
                ...
                // Merge rather than blind-replace: the server response is the
                // source of truth and already correctly ordered (oldest→newest).
                // Any optimistic message that hasn't been confirmed yet (e.g. a
                // reply sent in the split second before this fetch resolved)
                // gets appended AFTER it — always last, since it's guaranteed to
                // be the newest thing regardless of what timestamp it was given
                // client-side. We previously re-sorted the merged array by
                // created_at, but optimistic messages use the client's own
                // clock (`new Date().toISOString()`), and any clock skew vs the
                // server could sort a just-sent reply into the wrong position
                // relative to its actual neighbors — which is what was showing
                // up as replies not grouping with the right messages.
                setMessages(prev => {
                    const stillPending = prev.filter(
                        m => m.isOptimistic && !serverMsgs.some(sm => sm.id === m.id)
                    )
                    return [...serverMsgs, ...stillPending]
                })
                setHasMore(serverMsgs.length >= PAGE_SIZE)

                // Calibrate against the most recent confirmed message we have.
                if (serverMsgs.length > 0) {
                    const latest = serverMsgs[serverMsgs.length - 1]
                    clockOffsetRef.current = new Date(latest.created_at).getTime() - Date.now()
                }
                ...
                notifications.setUnreadMessagesCount((c) => Math.max(0, c - 1))
        ...
        // Realtime Subscription — consumes the dm:user:<uid> broadcast channel
        // (populated by chat_messages_broadcast_trigger) that InboxClient
        // already listens on for session-list updates, instead of a separate
        // postgres_changes subscription scoped just to this thread. That
        // channel carries every session's messages for this user, so each
        // handler below filters down to this specific session_id.
        let channel: RealtimeChannel | null = null
        let cancelled = false

        const setupMessageChannel = async () => {
            // Realtime's auth is now kept warm app-wide by ClientLayout (set
            // once at the root on load, and refreshed on every token
            // rotation), so we no longer block channel creation on an
            // awaited setAuth() call here — that per-mount round trip was
            // the main source of DMs lagging noticeably behind
            // confessions/etc. Fire a non-blocking call as a defensive
            // fallback only (e.g. a cold deep-link straight into a thread
            // before that root effect has finished), without waiting on it —
            // it's sent over the same socket ahead of the join below either way.
            supabase.realtime.setAuth().catch(() => { })

            if (cancelled) return

            channel = supabase
                .channel(`dm:user:${currentUser.id}`, { config: { private: true } })
                .on('broadcast', { event: 'INSERT' }, (msg) => {
                    const newMsg = msg.payload?.record
                    if (!newMsg || newMsg.session_id !== sid) return
                    ...
                    setMessages(prev => {
                        ...
                        // Keep the clock offset current — this is a genuinely
                        // confirmed (server) timestamp, so it's a reliable
                        // calibration point for the next optimistic send.
                        clockOffsetRef.current = new Date(newMsg.created_at).getTime() - Date.now()
                        ...
                        if (existingOptimisticIndex !== -1) {
                            ...
                            const oldTempId = prev[existingOptimisticIndex].id
                            reconcileReplyingToId(oldTempId, newMessageObj.id)
                ...
                // Deletes weren't mirrored anywhere before — a message removed by
                // the other participant (or a moderation action) would vanish
                // from their screen but persist forever in this device's Dexie
                // cache, so it could reappear on next load before the network
                // fetch caught up. Same fix pattern as confessions' DELETE handler.
                .on('broadcast', { event: 'DELETE' }, (msg) => {
                    const oldRec = msg.payload?.old_record
                    if (!oldRec || oldRec.session_id !== sid) return

                    const deletedId = oldRec.id
                    setMessages(prev => prev.filter(m => m.id !== deletedId))
                    db.chatMessages.delete(deletedId).catch(() => { })
                })
                .subscribe()
        }
        ...
        // Online status now comes from the app-wide PresenceProvider (see
        // isPartnerOnline below) instead of a channel scoped to this thread —
        // that's what makes a user show "online" while they're anywhere in
        // the app, not only when they happen to have this exact chat open.
        // last_active_at is heartbeated globally too (PresenceContext.tsx);
        // just read the partner's persisted value here as the "Offline · Xm
        // ago" fallback for whenever they're not currently online.
        supabase
            .from('profiles')
            ...
            .maybeSingle()
            .then(({ data }) => {
                if (data?.last_active_at) setPartnerLastSeen(new Date(data.last_active_at))
            })
            .catch(() => { })

        // This channel is now ONLY for the typing indicator, which is
        // legitimately scoped to this specific conversation.
    ...
    // Live-updating "last seen" fallback: whenever the global online set
    // flips this partner from online -> offline while the thread is open,
    // stamp it with "now" (more accurate than whatever's in the DB, which
    // only updates every 45s). When they come back online, clear it so we
    // don't show a stale label once isPartnerOnline flips back to true.
    const isPartnerOnline = isOnline(targetProfile.id)
    const wasPartnerOnlineRef = useRef(isPartnerOnline)
    useEffect(() => {
        if (wasPartnerOnlineRef.current && !isPartnerOnline) {
            setPartnerLastSeen(new Date())
        } else if (!wasPartnerOnlineRef.current && isPartnerOnline) {
            setPartnerLastSeen(null)
        }
        wasPartnerOnlineRef.current = isPartnerOnline
    }, [isPartnerOnline])
    ...
    const handleSend = async (overrideContent?: string) => {
        ...
        if (!sessionId) {
            ...
            const optimisticMsg: Message = {
                ...
                created_at: new Date(Date.now() + clockOffsetRef.current).toISOString(),
        ...
        try {
            ...
            if (result.success && result.data) {
                reconcileReplyingToId(tempId, result.data.id)
                setMessages(prev => {
    ...
    return (
        <div
            className="flex flex-col bg-neutral-950 text-neutral-200 overflow-hidden fixed inset-x-0"
            style={{
        ...
        >
            ...
            <div className="flex-shrink-0 px-4 py-3 bg-neutral-900/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between sticky top-0 z-50">
                            ...
                            <div className="flex items-center gap-1.5">
                                {targetProfile.is_deactivated ? (
                                    ...
                                    <>
                                        ...
                                        <span className={`text-[10px] font-bold uppercase tracking-wider ${isPartnerOnline ? 'text-emerald-500/80' : 'text-neutral-600'}`}>
                                            {isPartnerOnline
                                                ? 'Online'
                                                : partnerLastSeen
                                                    ? `Offline · ${formatLastSeen(partnerLastSeen)}`
                                                    : 'Offline'}
                                        </span>
                                    </>
                                )}
                ...
                <div className="flex items-center gap-1 relative" ref={menuRef}>
                    {isSyncing && (
                        <Loader2 size={14} className="animate-spin text-neutral-500 mr-1" />
                    )}
                    <button
                            ...
                            return (
                                <div key={msg.id} className="relative group">
                                    ...
                                    <motion.div
                                        ...
                                        onDragEnd={(e, info) => {
                                            if (info.offset.x > 50) {
                                                // It's fine to start a reply on a still-sending
                                                // (optimistic) bubble — replying to something you just
                                                // sent is a normal pattern, and reconcileReplyingToId
                                                // (see where replyingTo is declared) patches this in
                                                // place with the real id the moment the server confirms
                                                // it, so the reply link that actually gets persisted is
                                                // always a real one.
                                                setReplyingTo(msg)
                                                // Focusing synchronously here (mid-drag) was interrupting
                                                // the touch sequence on mobile and forcing the keyboard
                                                // open while framer-motion was still resolving the
                                                // snap-back animation — that's what was producing the
                                                // layout thrash (blank gap above the keyboard, bubbles
                                                // briefly reflowing/looking ungrouped). Deferring one
                                                // frame lets the drag gesture finish cleanly first.
