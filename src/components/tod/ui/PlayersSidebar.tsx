      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {activeTab === 'activity' && (
          <div className="p-4 flex flex-col h-full">
            {gameEvents.length > 0 ? (
              <>
                {/* Fixed-height frame sized for a full page of cards, so the
                    list settling in with fewer items (last page) or a new
                    real-time event landing on top never shifts anything below it. */}
                <div className="space-y-2" style={{ minHeight: `${activityPageSize * 62}px` }}>
                ...
                {/* Pagination — same structure every time (arrows always present,
                    just disabled at the edges) so nothing swaps layout as the
                    event count crosses the one-page threshold. */}
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-800/50 flex-shrink-0">
                  <button
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide tabular-nums">
                  ...
                  {/* Per-page size */}
                  <div className="flex items-center gap-1 ml-2 pl-2 border-l border-slate-800/50">
