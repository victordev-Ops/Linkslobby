...
interface WaitingRoomProps {
  ...
  // Bumped by the parent whenever a friendship becomes 'accepted', so an
  // already-rendered search result can flip from "Sent" to "Friends" live.
  friendsRefreshSignal?: number;
}
...
export const WaitingRoom = ({
  ...
  // Re-check the currently visible search results against the server whenever
  // the parent tells us a friendship was just accepted. Without this, a row
  // stays stuck on "Sent" until the user retypes the search.
  useEffect(() => {
    if (friendsRefreshSignal === 0) return; // skip initial mount
    if (!onSearchUsers) return;
    const q = searchQuery.trim();
    if (q.length < 2) return;

    onSearchUsers(q).then(setSearchResults).catch(() => {});
  }, [friendsRefreshSignal]);
