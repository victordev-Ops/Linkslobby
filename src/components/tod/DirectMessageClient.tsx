...
export default function DirectMessageClient({ sessionId: initialSessionId, isDraft = false, currentUser, targetProfile }: DirectMessageClientProps) {
    ...
    // Keep layout height in sync with the visual viewport so the on-screen
    // keyboard never covers the input area. 100dvh alone is unreliable on
    // iOS/Android when the keyboard animates in — visualViewport fires more
    // consistently and lets us pin the exact usable height.
    useEffect(() => {
    ...
    return (
        <div
            className="flex flex-col bg-neutral-950 text-neutral-200 overflow-hidden"
                                ...
                                <div key={msg.id} className="relative group">
                                    ...
                                    <motion.div
                                        ...
                                        onDragEnd={(e, info) => {
                                            if (info.offset.x > 50) {
                                                setReplyingTo(msg)
                                                // Swiping doesn't count as a tap on the input, so the
                                                // keyboard wouldn't otherwise open — focus explicitly.
