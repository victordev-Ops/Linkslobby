    ...
    useEffect(() => {
        ...
        presenceChannel
            .on('presence', { event: 'sync' }, () => {
                ...
                // Previously only 'join'/'leave' deltas updated isPartnerOnline,
                // so if the partner was ALREADY online when this chat mounted,
                // 'sync' never checked presenceState() for them and they'd show
                // "Offline" until something changed their presence later. Compute
                // it directly from the synced state every time instead.
                setIsPartnerOnline(flat.some(u => u.user_id === targetProfile.id))
