                        ...
                        {groupMessages(messages).map((msg, i, grouped) => {
                            ...
                            return (
                                <div key={msg.id} className="relative group">
                                    ...
                                    <motion.div
                                        drag={msg.isOptimistic ? false : "x"}
                                        dragConstraints={{ left: 0, right: 0 }}
                                        dragElastic={{ right: 0.15 }}
                                        onDragEnd={(e, info) => {
                                            if (info.offset.x > 50) {
                                                // A still-sending bubble only has a temp-* client id —
                                                // not a real, persisted one. Capturing that into
                                                // replyingTo and sending it as reply_to_id is what made
                                                // replies unreliable: the link either fails outright, or
                                                // (worse) gets stored but never matches a real row, so
                                                // the reply context silently disappears the next time
                                                // messages are re-fetched from the server (e.g. on
                                                // refresh). `drag={false}` above stops the gesture
                                                // entirely for these, and this is the belt-and-suspenders
