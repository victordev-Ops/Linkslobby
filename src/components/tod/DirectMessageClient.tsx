    ...
    useEffect(() => {
        ...
        const loadInitial = async () => {
            ...
            if (result.success && result.data) {
                ...
                // source of truth for anything it returned, but if the cache
                // briefly rendered a message the server call hasn't caught up
                // to yet (e.g. an optimistic send that just landed), keep it
                // instead of yanking it off screen for a frame. This is also
                // what keeps message grouping stable across the cache->server
                // handoff — a full replace with a differently-ordered/timed
                // array was flipping isFirstInGroup/isLastInGroup between renders.
                setMessages(prev => {
                    const byId = new Map(prev.map(m => [m.id, m]))
                    serverMsgs.forEach(m => byId.set(m.id, m))
        ...
        // Fallback for the relative "last seen" label: if the partner was
        // already offline before this chat was even opened, there's no live
        // presence 'leave' transition to capture it from during this session
        // — it would just show a bare "Offline" forever. Seed it from a
        // persisted timestamp instead. Requires a `last_active_at` column on
        // profiles (see accompanying SQL) that gets heartbeated below.
        supabase
            .from('profiles')
        ...
        // Heartbeat our own last_active_at so OTHER users get an accurate
        // fallback when they open a chat with us while we're offline.
        ...
        presenceChannel
            .on('presence', { event: 'sync' }, () => {
                // 'sync' is the single source of truth here — it fires after every
                // join/leave/track and reflects the fully-settled presence state.
                // Previously this ran ALONGSIDE separate 'join'/'leave' handlers
                // that also called setIsPartnerOnline directly; since 'leave' fires
                // before the corresponding 'sync' settles, the two could race and
                // flip the Online/Offline label back and forth. Using only 'sync'
                // removes that race.
                const state = presenceChannel.presenceState()
                const flat = Object.values(state).flat() as any[]
                const partnerPresent = flat.some(u => u.user_id === targetProfile.id)

                setIsPartnerOnline(prevOnline => {
                    if (partnerPresent && !prevOnline) {
                        setPartnerLastSeen(null) // clear stale "last seen" once they're back
                    } else if (!partnerPresent && prevOnline) {
        ...
        return () => {
            ...
            heartbeat() // final timestamp on the way out
