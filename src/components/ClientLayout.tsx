...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  // there's a session — this is the same gap that used to show as a blank
  // screen (see page.tsx `hasSession` fallback bug). Never re-armed after
  // the first resolve, so navigations and token refreshes don't retrigger it.
        ...
        <NotificationProvider profileId={profileId}>
          <XPNotificationProvider>
            <div className="min-h-screen transition-all">
              ...
              */}
              {/*
                visibleToasts caps how many stack up at once — without it,
                a burst of realtime events (several XP transactions, a
                friend request, a turn notification, all landing within a
                second of each other) queues an unbounded pile of toasts.
                richColors was removed: every toast in the app now renders
                through toast.custom() (via showAppToast/showXPNotification),
                so richColors' built-in palette never actually applied to
                anything — it was a no-op left over from before that
                unification.
              */}
              {/*
                expand keeps stacked toasts fully separated (each one fully
                visible, spaced by `gap`) instead of collapsing into
                sonner's default peek-stack, which is what read as
                "overlapping" when several toasts landed at once.
              */}
              <Toaster position="top-center" gap={10} visibleToasts={3} expand />
