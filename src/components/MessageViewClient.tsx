...
const CARD_VERTICAL_PADDING = 32 + 24 // pt-8 + pb-6 reserved, in px
const WATERMARK_RESERVED = 36       // approx height watermark row + its top gap takes, in px
const FONT_MAX = 1.5                // rem — short-message size (unchanged from before)
        ...
        >
          ...
          <div ref={shareWrapperRef} className="p-4 bg-transparent">
            <div className="w-full rounded-[2.5rem] overflow-hidden shadow-2xl bg-white border border-gray-100">
              ...
              <div
                className="relative px-6 pt-8 pb-6 flex flex-col items-center bg-white overflow-hidden"
          ...
          {/* Sender hint reveal — intentionally outside the capture box: it's
              private info, not meant for the shareable image, and shouldn't
              be constrained by the card's fixed height. */}
          {hint && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-4 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-500/20"
            >
              <p className="text-xs font-black text-purple-500 dark:text-purple-400 uppercase tracking-widest mb-3">
                🕵️ Sender Hints
              </p>
