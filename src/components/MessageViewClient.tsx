...
const CARD_CONTENT_HEIGHT = 440     // fixed px height for the text area — sized to fit 1000 chars at the font floor
const FONT_MAX = 1.5                // rem — short-message size (unchanged from before)
const FONT_MIN = 0.625              // rem — hard floor, still legible
const FONT_STEP = 0.025             // rem — finer granularity, lands closer to true fit
...
function useAutoFitFontSize(
  ...
  useLayoutEffect(() => {
    ...
    // Step down until the text's natural height fits the fixed box,
    // or we hit the floor (then we let CSS line-clamp + fade handle it).
    while (el.scrollHeight > containerHeight && size > FONT_MIN) {
  ...
  // true if even the floor size couldn't make it fit
  const overflowing =
    textRef.current ? textRef.current.scrollHeight > containerHeight && fontSize <= FONT_MIN : false
...
export default function MessageViewClient({ confession, username, onClose, restrictedWords = [], onDeleted, showWatermark = true }: Props) {
  ...
  const { fontSize, overflowing } = useAutoFitFontSize(
    textRef,
    CARD_CONTENT_HEIGHT,
    [cleanedMessage]
  )
        ...
        >
          ...
          <div ref={shareWrapperRef} className="p-4 bg-transparent">
            <div className="w-full rounded-[2.5rem] overflow-hidden shadow-2xl bg-white border border-gray-100">
              ...
              <div
                className="relative px-6 flex flex-col items-center justify-center bg-white"
                style={{ height: CARD_CONTENT_HEIGHT }}
              >
                <p
                  ref={textRef}
                  className="text-center text-gray-800 font-bold break-words whitespace-pre-wrap w-full"
                  style={{
                    fontSize: `${fontSize}rem`,
                    lineHeight: 1.4,
                    // last-resort clamp if even the floor size overflows
                    ...(overflowing
                      ? {
                          display: '-webkit-box',
                ...
                {overflowing && (
                  // fade-out so a clamped message reads as an intentional
