...
// --- FIXED-HEIGHT CARD, MEASURED AUTO-FIT (mobile-first, story-safe) ---
// Instead of growing the card to fit the text (which produces tall,
// awkward story images), we lock the card to a fixed content height and
// shrink the font until the text actually fits inside it. This is accurate
// for any mix of words/line-breaks/emoji, not just a char-count guess.
const CARD_CONTENT_HEIGHT = 380     // fixed px height for the whole content box (reduced from 440)
const CARD_VERTICAL_PADDING = 16 + 24 // pt-4 + pb-6 reserved, in px (gap halved, see card markup)
const WATERMARK_RESERVED = 36       // approx height watermark row + its top gap takes, in px
const FONT_MAX = 1.7                // rem — bumped up to use the reclaimed header-gap space
const FONT_MIN = 0.5                // rem — true safety floor, only for pathological input (e.g. all line breaks)
const FONT_PRECISION = 0.01         // rem — binary search resolution
...
function useAutoFitFontSize(
  ...
  useLayoutEffect(() => {
    ...
    // Binary search for the largest font size whose rendered height fits
    // the box. More reliable than stepping down by a fixed increment,
    // and converges to the true fit rather than stopping short of it.
    ...
    let lo = FONT_MIN
    let hi = FONT_MAX
    let best = fits(hi) ? hi : lo // if even the floor doesn't fit, best-effort = floor

    if (!fits(hi)) {
      // Fixed iteration count, not a float-difference threshold — this
      // guarantees termination even if rounding causes mid to repeat lo/hi.
      const MAX_ITERATIONS = 12
      for (let i = 0; i < MAX_ITERATIONS; i++) {
        const mid = Math.round(((lo + hi) / 2) * 100) / 100
        if (mid === lo || mid === hi) break // no more progress possible at this precision
        if (fits(mid)) {
          best = mid
          lo = mid
        } else {
...
export default function MessageViewClient({ confession, username, onClose, restrictedWords = [], onDeleted, showWatermark = true }: Props) {
  ...
  const isShortMessage = cleanedMessage.length <= 120
  const availableTextHeight =
    CARD_CONTENT_HEIGHT - CARD_VERTICAL_PADDING - (showWatermark ? WATERMARK_RESERVED : 0)
    ...
    <div className="fixed inset-0 bg-gray-50 dark:bg-[#0f0a1e] font-sans overflow-y-auto z-[60] transition-colors">
      ...
      <div className="flex flex-col items-center px-6 pb-24 relative z-10">
        <motion.div
          ...
          className="w-full"
          style={{ maxWidth: 'min(24rem, 92vw)', margin: '0 auto' }}
        >
          {/* THE CAPTURE AREA */}
          <div ref={shareWrapperRef} className="p-4 bg-transparent">
            <div className="w-full rounded-[2.5rem] overflow-hidden shadow-2xl bg-white border border-gray-100">
              ...
              <div
                className="relative px-6 pt-4 pb-6 flex flex-col items-center bg-white overflow-hidden"
                style={{ height: CARD_CONTENT_HEIGHT }}
              >
                <div className="flex-1 w-full flex items-center justify-center min-h-0">
                  <p
                    ref={textRef}
                    className={`text-gray-800 font-bold break-words whitespace-pre-wrap w-full ${
                      isShortMessage ? 'text-center' : 'text-left tracking-normal font-semibold'
                    }`}
                    style={{
          ...
          {/* Sender hint modal — separate from the card on purpose: it's
              private info, not meant for the shareable image. */}
          <AnimatePresence>
            {isHintModalOpen && hint && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[80] bg-black/50 backdrop-blur-sm flex items-center justify-center px-6"
              ...
              >
                <motion.div
                  ...
                  className="w-full max-w-sm bg-white dark:bg-[#1a1429] rounded-3xl border border-purple-100 dark:border-white/10 shadow-2xl p-6"
                >
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-xs font-black text-purple-500 dark:text-purple-300 uppercase tracking-widest">
                      🕵️ Sender Hints
                    </p>
                    <button
                      onClick={() => setIsHintModalOpen(false)}
                  ...
                  <div className="space-y-3">
                    {Object.entries(hint).map(([key, value]) => (
                      <div key={key} className="flex justify-between items-center gap-4">
                        <span className="text-xs font-bold text-purple-500 dark:text-purple-300 uppercase tracking-wider whitespace-nowrap">{key}</span>
