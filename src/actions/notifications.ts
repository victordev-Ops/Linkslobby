...
//
// FIX: 'lobby_join_response' and 'hot_seat_answer' now belong here too. The
// hidden_notifications check constraint has allowed both values for a while
// (confirmed live on the DB), but this array never caught up — so every delete
// attempt on those two types hit the same "can't be deleted" branch below, and
// isHideable() in NotificationsClient.tsx didn't even render a trash icon for
// them. Nothing about either type actually "resolves on its own" — they just
// sat there permanently unread/undeletable. They already use
// NOTIFICATION_READS_TYPES for marking read, same dual-tracking pattern as
// 'lobby'/'friend_request'/'friend_request_response' above.
const DELETABLE_TYPES: NotificationType[] = [
  ...
  'lobby_join_response', 'hot_seat_answer'
]
...
const NOTIFICATION_TYPE_KEY: Record<string, string> = {
  ...
  lobby: 'lobby_event',
  // These two already match their own check-constraint value 1:1 — listed
  // explicitly (rather than relying on notificationTypeKey()'s fallback)
  // so this map stays the single source of truth for every deletable type,
  // matching the mirrored copy in context/NotificationContext.tsx.
  lobby_join_response: 'lobby_join_response',
  hot_seat_answer: 'hot_seat_answer',
}
...
export async function bulkDeleteNotifications(items: BulkItem[]): Promise<BulkResult> {
  ...
  // Non-deletable types never reach the DB. (Currently everything is
  // deletable — this filter is kept as a safety net for any future type
  // that gets added to NotificationType before it's added to DELETABLE_TYPES.)
