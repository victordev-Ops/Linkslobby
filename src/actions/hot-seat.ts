...
export async function closeHotSeatSession(sessionId: string): Promise<ActionResult> {
    ...
    // Revalidate the list page and every /hot-seat/[slug] page (we only have
    // the id here, not the slug, so bust the whole segment via the layout).
    revalidatePath('/hot-seat', 'layout')
    return { success: true }
}
...
export async function deleteHotSeatSession(sessionId: string): Promise<ActionResult> {
    ...
    // Already gone (e.g. a stale client retried the delete, or two tabs both
    // fired it). The desired end state — no session — is already true, so
    // treat this as success instead of surfacing a misleading permissions
    // error and leaving an un-removable row in the UI.
    if (!session) {
        revalidatePath('/hot-seat', 'layout')
        return { success: true }
    }
